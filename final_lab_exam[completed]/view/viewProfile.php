...
if (!empty($_REQUEST['id'])) {
    ...
    // Redirect to a safe page (e.g., dashboard or login) if user ID is missing
    header('Location: viewProfile.php'); // Change to an appropriate page
    exit();
}
...
$user = getUser($userId);
if (!$user) {
    // Redirect to a safe page if the user is not found
    header('Location: viewProfile.php'); // Adjust this path
    exit();
}
